  ...
  public static function getDeactivateInfo($companyId){
    ...
    public static function profileCompleteness($cp){
     ...
     //4.5 each of of 22
      ...
      $runSum = (isset($cp[0]->number_of_employees) && strlen($cp[0]->number_of_employees) > 0)? ($runSum + $n) : ($runSum + 0);
      ...
      $runSum = (isset($cp[0]->financial_year) && strlen($cp[0]->financial_year) > 0)? ($runSum + $n) : ($runSum + 0);
      ...
      $awards = (count($cp[2]) > 0)? 100 : 0;

      $pinvoice = (count($cp[3]) > 0)? 100 : 0;

      $sinvoice = (count($cp[4]) > 0)? 100 : 0;

      $cert = (count($cp[5]) > 0)? 100 : 0;
    ...
    public static function profileStrengthMessages($cp){
      ...
      $runSum[] = (isset($cp[0]->registered_company_name) && strlen($cp[0]->registered_company_name) > 0)? null : "Add company registered number";
      ...
      $runSum[] = (isset($cp[0]->number_of_employees) && strlen($cp[0]->number_of_employees) > 0)? null : "Add company number of employees";
      ...
      $runSum[] = (isset($cp[0]->financial_year) && strlen($cp[0]->financial_year) > 0)? null : "Add financial year";
      ...
      $runSum[] = (count($cp[2]) > 0)? null : "Add company awards";

      $runSum[] = (count($cp[3]) > 0)? null : "Add company purchase invoice";

      $runSum[] = (count($cp[4]) > 0)? null : "Add company sales invoice";

      $runSum[] = (count($cp[5]) > 0)? null : "Add company certificates";
